  ...
  return (
    <div className="max-w-5xl animate-fade-in-up space-y-8">
      ...
      <div className="space-y-6">
        ...
        <div className="relative group rounded-2xl border border-white/5 bg-gradient-to-br from-[#0d1117] to-[#0a0f16] overflow-hidden hover:border-purple-500/20 transition-all duration-500 mb-6">
          <div className="absolute top-0 left-0 w-1 h-full bg-gradient-to-b from-purple-500 to-purple-500/0 rounded-l-2xl" />
          <div className="absolute -top-32 -right-32 w-64 h-64 bg-purple-500/[0.03] rounded-full blur-3xl group-hover:bg-purple-500/[0.06] transition-all duration-700" />
          <div className="relative p-6 md:p-8">
            <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
              <div className="flex-1 space-y-5">
                <div className="flex items-center gap-4">
                  <div className="relative">
                    <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-purple-500/20 to-purple-500/5 border border-purple-500/20 flex items-center justify-center shadow-[0_0_20px_rgba(168,85,247,0.12)]">
                      <svg className="w-7 h-7 stroke-purple-400 fill-none" viewBox="0 0 24 24" strokeWidth={2}>
                        <path strokeLinecap="round" strokeLinejoin="round" d="M10.5 1.5H8.25A2.25 2.25 0 006 3.75v16.5a2.25 2.25 0 002.25 2.25h7.5A2.25 2.25 0 0018 20.25V3.75a2.25 2.25 0 00-2.25-2.25H13.5m-3 0V3h3V1.5m-3 0h3m-3 18.75h3" />
                      </svg>
                    </div>
                    <span className="absolute -top-1 -right-1 w-3.5 h-3.5 bg-purple-500 rounded-full border-2 border-[#0d1117] shadow-[0_0_8px_rgba(168,85,247,0.5)]" />
                  </div>
                  <div>
                    <h3 className="text-base font-black text-white tracking-tight flex items-center gap-2">
                      Apple Shortcuts (iOS Webhook)
                      <span className="text-[10px] bg-purple-500/10 text-purple-400 border border-purple-500/20 px-2 py-0.5 rounded-full uppercase tracking-wider font-bold">
                        iPhone Ready
                      </span>
                    </h3>
                    <p className="text-[10px] text-purple-400/70 font-semibold uppercase tracking-widest mt-0.5">
                      1-Click iOS Automation • Real-Time Webhook
                    </p>
                  </div>
                </div>

                <p className="text-xs text-[--text-secondary] leading-relaxed max-w-lg">
                  Since iOS blocks background SMS reading, you can connect your iPhone using Apple Shortcuts! Whenever you close Google Pay, PhonePe, or Apple Wallet on your iPhone, your shortcut pops up asking for amount/category and posts directly to your ledger with instant Telegram budget warnings.
                </p>
                  ...
                  <div className="space-y-4">
                    <div className="p-4 rounded-xl bg-purple-500/5 border border-purple-500/10 space-y-2">
                      <p className="text-[10px] font-black uppercase tracking-wider text-purple-400">
                        Your Personal iOS Webhook URL
                      </p>
                      <div className="flex items-center justify-between gap-2 bg-black/40 p-2.5 rounded-lg border border-white/5">
                        <code className="text-xs font-mono text-[--text-secondary] truncate select-all">
                          {typeof window !== "undefined"
                            ? `${window.location.origin}/api/transactions/shortcut-sync?token=${profile.sms_sync_token}`
                            : `/api/transactions/shortcut-sync?token=${profile.sms_sync_token}`}
                        </code>
                        <button
                          type="button"
                          onClick={() => {
                            const url = `${window.location.origin}/api/transactions/shortcut-sync?token=${profile.sms_sync_token}`;
                            navigator.clipboard.writeText(url);
                            toast.success("iOS Webhook URL copied to clipboard!");
                          }}
                          className="px-3 py-1.5 rounded-lg bg-purple-500/10 hover:bg-purple-500/20 text-[11px] font-bold text-purple-300 transition-colors shrink-0"
                        >
                          Copy Webhook
                        </button>
                      </div>
                    ...
                    <div className="p-4 rounded-xl bg-white/[0.02] border border-white/5 space-y-2.5">
                      <p className="text-xs font-bold text-white flex items-center gap-2">
                        <span>📲</span> Quick iPhone Setup Guide (3 Steps)
                      </p>
                      <ol className="text-xs text-[--text-secondary] space-y-1.5 list-decimal list-inside leading-relaxed">
                        <li>Open the **Shortcuts app** on your iPhone $\rightarrow$ **Automations** $\rightarrow$ **Create Personal Automation**.</li>
                        <li>Choose **App** $\rightarrow$ select *Google Pay / PhonePe / Paytm* $\rightarrow$ check **Is Closed** $\rightarrow$ Run Immediately.</li>
                        <li>Add action **Ask for Input** (Text: "What did you spend? e.g. 450 Swiggy"), then add **Get Contents of URL** with method `POST` to your copied Webhook URL above!</li>
                      </ol>
                    </div>
                  </div>
                ) : (
                  <div className="p-4 rounded-xl bg-amber-500/5 border border-amber-500/10 text-xs text-amber-300">
                    Generate your sync token above in the **SMS / Notification Sync** section to enable your iOS Webhook.
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="relative p-6 md:p-8">
            <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
              <div className="flex-1 space-y-5">
                ...
                <p className="text-xs text-[--text-secondary] leading-relaxed max-w-md">
                  Your supercharged financial assistant on Telegram. Send Voice Notes (🎙️) or Bill Photos (📸) with captions, paste Bank SMS alerts for auto-categorization, or run slash commands (`/balance`, `/summary`, `/recent`, `/undo`) to manage your money instantly.
                </p>
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-2 mb-4">
                        {[
                          { cmd: "/balance", desc: "Net Worth & Accounts", color: "text-cyan-400" },
                          { cmd: "/summary", desc: "Monthly Report", color: "text-emerald-400" },
                          { cmd: "/recent", desc: "Last 5 Transactions", color: "text-blue-400" },
                          { cmd: "/undo", desc: "Revert Last Entry", color: "text-rose-400" },
                          { cmd: "/goals", desc: "Goals Progress", color: "text-purple-400" },
                          { cmd: "/budget", desc: "Budget Limits", color: "text-amber-400" },
                        ].map((c) => (
                          <div
                            key={c.cmd}
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                        {[
                          { cmd: "120+45+30 Lunch", desc: "Inline Math Expense", color: "text-rose-400" },
                          { cmd: "income 5000 Salary", desc: "Quick Income", color: "text-emerald-400" },
                          { cmd: "Paste Bank SMS", desc: "Auto-Categorize Alert", color: "text-cyan-400" },
                          { cmd: "stock buy 10 AAPL 150", desc: "Stock Trade", color: "text-blue-400" },
                          { cmd: "mf sip 5000 NIFTY", desc: "Mutual Fund", color: "text-purple-400" },
                          { cmd: "family sent 500 mom", desc: "Family Transfer", color: "text-pink-400" },
                        ].map((c) => (
                          <div
                            key={c.cmd}
