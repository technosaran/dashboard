...
export default function SettingsPage() {
  ...
  const handleSeedData = async () => {
    setIsSeeding(true);
    const toastId = toast.loading("Populating database with premium seed accounts & assets...");
    try {
    ...
    <div className="flex flex-col gap-[var(--section-gap)] animate-fade-in">
      ...
      <div className="max-w-2xl animate-fade-in-up delay-2">
        <div className="glass-card-static p-6 md:p-10 border-rose-500/20 bg-rose-500/[0.02]">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 p-5 rounded-2xl bg-emerald-500/[0.04] border border-emerald-500/15 mb-4">
            <div>
              ...
              <p className="text-xs text-[--text-muted] mt-1">
                Erase current data and generate a complete premium portfolio with sample accounts, stocks, bonds, and mutual funds for immediate demonstration.
              </p>
            </div>
            <button
              onClick={handleSeedData}
              disabled={isSeeding || isResetting}
              className="btn-success !h-11 !px-6 whitespace-nowrap shadow-xl shadow-emerald-500/20 flex items-center gap-2"
            >
              {isSeeding ? (
                <>
