  ...
  return (
    <div className="flex flex-col gap-[var(--section-gap)] animate-fade-in">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <button onClick={() => setShowForm(!showForm)} className={`btn-primary h-[48px] px-6 md:px-8 flex-1 md:flex-none flex items-center justify-center gap-2.5 rounded-2xl shadow-xl shadow-[--accent-primary]/25 transition-all hover:scale-105 active:scale-95 text-sm ${showForm ? "btn-secondary" : ""}`}>
      ...
      {accounts.length < 2 && (
        <div className="animate-fade-in-up delay-1" style={{ padding: "16px 20px", borderRadius: "var(--radius-lg)", background: "rgba(253, 203, 110, 0.08)", border: "1px solid rgba(253, 203, 110, 0.2)", marginBottom: "32px", display: "flex", alignItems: "center", gap: "12px" }}>
          <div style={{ width: "36px", height: "36px", borderRadius: "var(--radius-sm)", background: "rgba(253, 203, 110, 0.12)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.8} viewBox="0 0 24 24" style={{ color: "#fdcb6e" }}><path d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L4.082 16.5c-.77.833.192 2.5 1.732 2.5z" /></svg></div>
          <p className="text-sm" style={{ color: "#fdcb6e" }}>You need at least 2 accounts to make transfers. Create more accounts first.</p>
        </div>
      )}

      {showForm && accounts.length >= 2 && (
        <div className="glass-card-static animate-scale-in p-5 md:p-8 mb-8">
          <div className="flex items-center gap-3 mb-5"><div style={{ width: "36px", height: "36px", borderRadius: "var(--radius-sm)", background: "rgba(162, 155, 254, 0.12)", border: "1px solid rgba(162, 155, 254, 0.2)", display: "flex", alignItems: "center", justifyContent: "center" }}><svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.8} viewBox="0 0 24 24" style={{ color: "#a29bfe" }}><path d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4" /></svg></div><h2 className="text-lg font-bold" style={{ color: "var(--text-primary)" }}>New Transfer</h2></div>
          {error && (<div className="mb-5 animate-fade-in" style={{ padding: "12px 16px", borderRadius: "var(--radius-md)", background: "rgba(255, 71, 87, 0.08)", border: "1px solid rgba(255, 71, 87, 0.2)", color: "#ff6b81", fontSize: "0.8125rem" }}>{error}</div>)}
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="flex flex-col sm:flex-row gap-3 mt-6"><button type="submit" disabled={submitting} className="btn-primary h-[48px] px-8 flex-1 sm:flex-none flex items-center justify-center text-sm" style={{ opacity: submitting ? 0.6 : 1, cursor: submitting ? "not-allowed" : "pointer" }}>{submitting ? (<span className="flex items-center gap-2"><svg className="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v4a4 4 0 00-4 4H4z" /></svg>Processing...</span>) : ("Initiate Transfer")}</button><button type="button" onClick={resetForm} disabled={submitting} className="btn-secondary h-[48px] px-8 flex-1 sm:flex-none flex items-center justify-center text-sm" style={{ opacity: submitting ? 0.6 : 1 }}>Close</button></div>
          </form>
        </div>
      )}

      <div className="glass-card-static overflow-hidden animate-fade-in-up delay-2">
        <div className="flex items-center justify-between" style={{ padding: "20px 24px", borderBottom: "1px solid var(--border-subtle)" }}><div className="flex items-center gap-3"><div style={{ width: "32px", height: "32px", borderRadius: "var(--radius-sm)", background: "rgba(0, 206, 201, 0.1)", border: "1px solid rgba(0, 206, 201, 0.15)", display: "flex", alignItems: "center", justifyContent: "center" }}><svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={1.8} viewBox="0 0 24 24" style={{ color: "#00cec9" }}><path d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg></div><h2 className="text-lg font-bold" style={{ color: "var(--text-primary)" }}>Transfer History</h2></div><span className="text-xs font-medium px-3 py-1 rounded-full" style={{ background: "rgba(0, 206, 201, 0.1)", color: "#00cec9", border: "1px solid rgba(0, 206, 201, 0.15)" }}>{transfers.length} total</span></div>
