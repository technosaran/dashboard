...
interface StocksHistoryTableProps {
  trades: StockTrade[];
}

const columnHelper = createColumnHelper<StockTrade>();

export default function StocksHistoryTable({ trades }: StocksHistoryTableProps) {
  ...
  if (trades.length === 0) {
    return (
      <EmptyState 
        icon="📜"
        title="No history"
        description="You have no recorded stock trades yet."
      />
    );
  }
