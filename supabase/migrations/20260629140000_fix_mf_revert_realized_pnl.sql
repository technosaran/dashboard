-- Migration: Fix MF Revert realized_pnl Reference
-- Date: 2026-06-29
-- Purpose: In revert_ledger_log, the mutual_fund section incorrectly referenced v_mf_trade.units_reverted_pnl
-- which does not exist in the mutual_fund_trades schema. It should reference v_mf_trade.realized_pnl.
    ...
    ELSIF v_log.source_type = 'mutual_fund' THEN
        ...
        IF FOUND THEN
            ...
            ELSE
                UPDATE mutual_funds SET 
                    units = units + v_mf_trade.units, 
                    realized_pnl = COALESCE(realized_pnl, 0) - COALESCE(v_mf_trade.realized_pnl, 0), -- fixed column reference from units_reverted_pnl
                    updated_at = NOW()
                WHERE id = v_item.id;
            END IF;
