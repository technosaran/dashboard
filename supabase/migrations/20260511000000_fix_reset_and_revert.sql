...
DECLARE v_bank_bal DECIMAL; v_bank_name TEXT; v_forex_label TEXT; v_txn_id UUID;
BEGIN
  ...
  IF p_bank_account_id IS NOT NULL THEN
    SELECT balance, name INTO v_bank_bal, v_bank_name FROM accounts WHERE id = p_bank_account_id AND user_id = p_user_id FOR UPDATE;
    IF v_bank_bal < p_amount THEN RETURN json_build_object('success', false, 'error', 'Insufficient balance'); END IF;
    UPDATE accounts SET balance = balance - p_amount WHERE id = p_bank_account_id;
    INSERT INTO ledger_logs (user_id, account_id, account_name, action_type, amount, previous_balance, new_balance, details, source_id, source_type)
    VALUES (p_user_id, p_bank_account_id, v_bank_name, 'ADJUST_DOWN', p_amount, v_bank_bal, v_bank_bal - p_amount, 'Forex deposit to ' || v_forex_label, v_txn_id, 'forex');
  END IF;
...
DECLARE v_forex_bal DECIMAL; v_forex_label TEXT; v_bank_bal DECIMAL; v_bank_name TEXT; v_txn_id UUID;
BEGIN
  IF p_amount <= 0 THEN RETURN json_build_object('success', false, 'error', 'Amount must be positive'); END IF;
  ...
  IF p_bank_account_id IS NOT NULL THEN
    ...
    UPDATE accounts SET balance = balance + p_amount WHERE id = p_bank_account_id;
    INSERT INTO ledger_logs (user_id, account_id, account_name, action_type, amount, previous_balance, new_balance, details, source_id, source_type)
    VALUES (p_user_id, p_bank_account_id, v_bank_name, 'ADJUST_UP', p_amount, v_bank_bal, v_bank_bal + p_amount, 'Forex withdrawal from ' || v_forex_label, v_txn_id, 'forex');
  END IF;
