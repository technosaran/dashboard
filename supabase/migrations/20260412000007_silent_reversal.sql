-- Update revert_ledger_log for SILENT reversals (no reversal logs)
...
BEGIN
    ...
    -- 2. Handle Transfer Reversal (Silent)
    IF v_log.source_type = 'transfer' AND v_log.source_id IS NOT NULL THEN
        ...
        FOR v_other_log IN SELECT * FROM ledger_logs WHERE source_id = v_log.source_id AND source_type = 'transfer' AND user_id = p_user_id LOOP
            ...
            -- WE SKIP INSERTING A REVERSAL LOG HERE (AS REQUESTED)
        END LOOP;
    ...
    -- 3. Handle Single Log Reversal (Expense, Income, Adjustment, etc.) - SILENT
    IF v_log.account_id IS NOT NULL AND v_log.amount IS NOT NULL THEN
        ...
        IF FOUND THEN
            -- Logical Reversal based on action type
            ...
            ELSIF v_log.action_type IN ('ADJUST_UP', 'TRANSFER_IN') THEN
                v_rev_amount := -v_log.amount; -- Take money away
            ELSE
                v_rev_amount := 0;
            END IF;
            ...
            IF v_rev_amount != 0 THEN
                ...
                -- SILENT COMPLETION (No reversal log inserted)
            END IF;
        END IF;
    END IF;
    ...
    -- 5. Delete original log (Final trace removal)
