-- Update revert_ledger_log to handle income records
...
BEGIN
    ...
    -- 3. Handle Single Log Reversal (Expense, Income, Adjustment, etc.)
    ...
    IF v_log.source_id IS NOT NULL THEN
        ...
        ELSIF v_log.source_type = 'income' THEN
            -- CRITICAL: CLEANUP THE INCOME TABLE
            DELETE FROM incomes WHERE id = v_log.source_id AND user_id = p_user_id;
            -- Cleanup matching transaction record by looking for the one created alongside this income
